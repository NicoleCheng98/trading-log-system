...
class Trade(db.Model):
    ...
    # 复盘相关字段
    review_date = db.Column(db.DateTime, comment='复盘日期')
    execution_quality = db.Column(db.Integer, comment='执行质量评分(1-10)')
    psychology_state = db.Column(db.String(50), comment='交易心理状态')
    market_condition = db.Column(db.String(50), comment='市场条件')
    lessons_learned = db.Column(db.Text, comment='经验教训')
    improvement_points = db.Column(db.Text, comment='改进点')
    ...
    def to_dict(self):
        ...
        return {
            ...
            'trade_screenshots': self.trade_screenshots,
